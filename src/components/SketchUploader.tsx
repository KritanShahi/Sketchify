...
const generateEffect = async () => {
  ...
  const formData = new FormData();
  formData.append("image", image);
  formData.append("effect", effect);  // important! send the effect

  setLoading(true);

  try {
    const res = await axios.post("http://127.0.0.1:8000/api/sketch/", formData);
    setResult(res.data.image_base64);  // show result in UI
  } catch (err: any) {
  ...
  return (
    <FullPageContainer>
      ...
      <MainContainer>
        ...
        <Button onClick={generateEffect} disabled={loading || !image}>
  ...
  {loading ? "Uploading..." : "⬆️ Upload Image"}
        ...
        {result && (
          <ResultWrapper>
            ...
            <ButtonRow>
              <DownloadButton href={result} download={`${effect}.png`}>
                ⬇️ Download
              </DownloadButton>
              <ResetButton onClick={resetAll}>Upload Another</ResetButton>
            </ButtonRow>
          </ResultWrapper>
        )}
      </MainContainer>
